...
int main() {
    ...
    do {
        ...
        switch (opcionPrincipal) {
            case 1: {
    // Validar ID (solo números enteros no negativos)
    int id;
    do {
        printf("Ingrese el ID del producto: ");
        if (scanf("%d", &id) != 1 || id < 0) {
            printf("Error: El ID debe ser un número entero no negativo.\n");
    ...
    scanf(" %[^\n]s", productos[n].nombre); // Leer el nombre con espacios
    if (!validarNombre(productos[n].nombre)) {
        printf("Error: Solo se permiten caracteres alfabéticos en el nombre.\n");
        break;
    }

    // Validar Cantidad (solo números enteros no negativos)
    int cantidad;
    do {
        printf("Ingrese la cantidad del producto: ");
        if (scanf("%d", &cantidad) != 1 || cantidad < 0) {
            printf("Error: La cantidad debe ser un número entero no negativo.\n");
            while (getchar() != '\n'); // Limpiar el buffer de entrada
        } else {
            break;
        }
